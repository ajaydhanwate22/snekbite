...
function RescuerCreateNewPassword({ navigation,route }) {
      ...
      .then(data => {
        if (data.success) {
          Alert.alert(t('Success'), data.message, [
            {
              text: t('OK'),
              onPress: () => navigation.navigate('RescuerLoginScreen'), // Navigate after successful password update
            },
          ]);
        } else {
...
export default RescuerCreateNewPassword;
