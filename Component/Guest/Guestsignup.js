...
function Guestsignupscreen({navigation}) {
  ...
  const handleButtonPress = async () => {
    if (!username ||  !contactnumber || !email || !address || password !== confirmPassword ||  !education)  
      {Alert.alert(
        'Error',
    ...
    try {
      const response = await axios.post(
        'https://realrate.store/ajayApi/Guestsignup.php',
        formData,
        {
          headers: {
  ...
  return (
    ...
    <ImageBackground source={require('../Assets/background.png')} 
     ...
     <View style={{width: '100%', height: 880, backgroundColor: 'white', top: -50, borderRadius: 30, marginBottom: -30, elevation: 5, padding: 20, gap: 20}}>
     ...
     <View style={{ flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#093624', borderRadius: 10, paddingLeft: 15, gap: 20, width: '100%', height: 55 }}>
         ...
         <TextInput placeholder={t('Username')} placeholderTextColor="#093624" value={username} onChangeText={text => { const capitalizedText = text.charAt(0).toUpperCase() + text.slice(1).toLowerCase(); setusername(capitalizedText); }} style={{ flex: 1 }} />
     </View>
...
{/* 
         ...
         <View style={{ flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#093624', borderRadius: 10, paddingLeft: 15, gap: 20, width: '100%', height: 55 }}>
           ...
           <TextInput placeholder={t('Expertise/Profession')} placeholderTextColor="#093624" value={education} onChangeText={setEducation} style={{ flex: 1 }} />
         </View>
